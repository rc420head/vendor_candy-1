...
if [ "$1" ]; then
    ...
    DEVICE=$(echo $TARGET_PRODUCT | sed 's/candy_//g')
    if [ -f $file_path ]; then
        ...
        if [[ $file_name == *"OFFICIAL"* ]] || [[ $FORCE_JSON == 1 ]]; then
            ...
            build_type=$(grep ro\.candy\.buildtype ./out/target/product/$DEVICE/system/build.prop | cut -d= -f2);
            base_version=$(grep org\.candy\.version ./out/target/product/$DEVICE/system/build.prop | cut -d= -f2);
            link="https://sourceforge.net/projects/candyroms/files/Official/14/${device}/${file_name}/download"
            echo "{" > $file_path.json

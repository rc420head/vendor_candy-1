function __print_candy_functions_help() {
cat <<EOF
...
- candygerrit:   A Git wrapper that fetches/pushes patch from/to LineageOS Gerrit Review.
- candyrebase:   Rebase a Gerrit change and push it again.
- candyremote:   Add git remote for LineageOS Gerrit Review.
...
function breakfast()
{
    target=$1
    local variant=$2
    local aosp_target_release=$(cat ${ANDROID_BUILD_TOP}/vendor/candy/vars/aosp_target_release 2>/dev/null)
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            lunch candy_$target-$aosp_target_release-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/candy-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.candy.device | grep -q "$CANDY_BUILD"); then
...
function candyremote()
{
    ...
    local CANDY_USER=$(git config --get review.review.candyos.org.username)
    if [ -z "$CANDY_USER" ]
    then
        git remote add CANDY ssh://review.candyos.org:29418/$PFX$PROJECT
    else
        git remote add candy ssh://$CANDY_USER@review.candyos.org:29418/$PFX$PROJECT
    fi
...
function makerecipe() {
    ...
    then
        pwd
        candyremote
        git push candy HEAD:refs/heads/'$1'
    fi
    '
}
...
function candygerrit() {
    ...
    local user=`git config --get review.review.candyos.org.username`
...
function candyrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        ...
        echo "      candyrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.candyos.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then

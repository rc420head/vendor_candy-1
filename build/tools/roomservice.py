...
#           (C) 2017-2018,2020-2021, CandyRoms
#
...
if not depsonly:
    print("Device %s not found. Attempting to retrieve device repository from CandyRoms Github (http://github.com/CandyRoms)." % device)
...
if not depsonly:
    githubreq = urllib.request.Request("https://raw.githubusercontent.com/CandyRoms/mirror/master/default.xml")
    try:
...
def is_in_manifest(projectpath):
    ...
    # ... and don't forget the candy snippet
    try:
        lm = ElementTree.parse(".repo/manifests/snippets/candy.xml")
        lm = lm.getroot()
    except:
        lm = ElementTree.Element("manifest")
...
def add_to_manifest(repositories):
    ...
    for repository in repositories:
        ...
        if is_in_manifest(repo_target):
            print('CandyRoms/%s already fetched to %s' % (repo_name, repo_target))
            continue

        print('Adding dependency: CandyRoms/%s -> %s' % (repo_name, repo_target))
        project = ElementTree.Element("project", attrib = {
            "path": repo_target,
            "remote": "github",
            "name": "CandyRoms/%s" % repo_name,
...
def fetch_dependencies(repo_path):
    ...
    dependencies_path = repo_path + '/candy.dependencies'
    syncable_repos = []
    verify_repos = []
...
def get_default_or_fallback_revision(repo_name):
    ...
    githubreq = urllib.request.Request("https://api.github.com/repos/CandyRoms/" + repo_name + "/branches")
    add_auth(githubreq)
...
print("Repository for %s not found in the CandyRoms Github repository list. If this is in error, you may need to manually add it to your local_manifests/roomservice.xml." % device)
